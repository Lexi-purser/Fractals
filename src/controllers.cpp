...
/*int buck(ActionData& action_data){
    int quest = askUncleBuckQuestions(action_data);
    return quest;
}*/
...
void showMenu( MenuData& menu_data, ActionData& action_data ){
    ...
    for(i = 0; i < menu_data.getNames().size(); i++){
        std::vector<std::string> c_names = menu_data.getNames();
        action_data.getOS() << c_names[i] << ") " << menu_data.getDescription(c_names[i]) << "\n";
    }
}
...
void takeAction(const std::string& choice, MenuData& menu_data, ActionData& action_data){
    ...
    if(menu_data.getFunction(choice) != 0){
        //std::cout << menu_data.getFunction(choice) << std::endl;
...
//menudata add action format cp menu_data.addAction("", ,"");
...
int imageMenu(std::istream& is, std::ostream& os){
    ...
    while(action_data.getIS().good() == true && action_data.getDone() != true){
